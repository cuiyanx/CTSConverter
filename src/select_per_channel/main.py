...
def transfer(ipath, opath):
  names_dict = get_file_names(ipath, ".py")

  for (file_name, file_path) in names_dict.items():
    file_path = file_path[len(ipath)+1:-len(file_name)-1]

    input_dir = os.path.join(ipath, file_path)
    output_dir = os.path.join(opath, file_path)
    ...
    output_file = os.path.join(output_dir, file_name[:-6] + "js")

    assert os.path.exists(input_file), "input file is not exist: %s"%input_file

    cmd = "python3 ./src/select_per_channel/cts_generator.py " + input_file +\
          " -js " + output_file
    os.system(cmd)
...
if __name__ == "__main__":
  ...
  if not args_transfer == "-":
    ...
    transfer(args_transfer, output_path_transfer)
